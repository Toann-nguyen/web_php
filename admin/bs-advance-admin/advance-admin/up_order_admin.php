        ...
        <div id="page-wrapper">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
               <div class="panel panel-info">
                        ...
                        <?php
     include('../advance-admin/control.php');
      $get_data=new data_admin();          
     if(isset($_GET['up'])){
        $select_order = $get_data->select_order_pro_id($_GET['up']);
    //lấy dữ liệu từ trang select
    //$_GET['up']là giá trị truyền từ trang select sang
    foreach ($select_order as $i_pr){
    ?>
                            <form role="form" method="POST" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label>NAME PRODUCT</label>
                                            <input class="form-control" type="text" name="txt_tinhtrang" value="<?php echo $i_pr['tinhtrang']?>" >
...
<?php
    ...
    if(isset($_POST['txtsub'])){// kiểm tra nút đăng ký đã ấn hay chưa
        // $up=$get_data->update_product($_POST['txtNP'],$_POST['txtNUP'],$_FILES['txtPIP']['name'],$_POST['txtCP'],$_POST['txtDP'],$_POST['txtPP'],$_POST['txtDEP'], $_GET['up']);
            $up = $get_data->update_order_pro($_POST['txt_tinhtrang'],$_GET['up']);
        if($up) {
            echo"<script> alert('Thanh cong')
        window.location='select_order_admin.php';
        </script>";
