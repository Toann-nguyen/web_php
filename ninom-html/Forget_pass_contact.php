          ...
          <div class="col-lg-4 offset-lg-2 col-md-5 offset-md-1">
            <form action="" role="form" method="POST" enctype="multipart/form-data">
              <div class="contact_form-container">
                <div>
                  <div>
                    <input value="" type="email" placeholder="email" name="txtemail" />
                  </div>
                  <div>
                    <button name="send_email" type="submit">
                      Send
                    </button>
                  </div>
                </div>
            ...
            include('../admin/bs-advance-admin/mail.php');
            $mail = new email();
            
            if (isset($_POST['send_email'])) {
              $email = $_POST['txtemail'];
              $forget_pas = new data_user();
              
              $select_password_send_email = $forget_pas->select_user_foeget_password($email);
                      if ($row = mysqli_fetch_assoc($select_password_send_email)) {
                  if (isset($row['password'])) {
                      $email=$_POST['txtemail'];
                      // check email trong database
                      $pass = $forget_pas->send_pass($email);
                      // goi ham gui email 
                      $mail->push_email($email, $pass);
                      echo "alert('Gửi mail thành công')";
                      echo "<script>window.location.href = 'login_contact.php';</script>";
                  } else {
